...
export function RegisterPage() {
  ...
  const handleOtp = async (code) => {
    ...
    try {
      ...
      setError(uploadErr.message || "Account created but photo upload failed.");
      setLoading(false);
  ...
  if (otpStep) {
    return (
      <AuthPageShell title="Verify Email" hint="Enter the code we sent to complete registration." tagline="Join Verdalis Foods" showValues footer={<AuthSignInLink />}>
        <OtpVerifyStep
          email={otpEmail}
          onVerify={handleOtp}
  ...
  return (
    <AuthPageShell
      ...
      hint="Shop with standard pricing and track orders in your dashboard."
      tagline="Join Verdalis Foods"
      showValues
      footer={
        <>
          <AuthSignInLink />
          <p className="ck-login-signup">
            Business wholesale? <Link to="/register/retailer">Retailer registration</Link>
          </p>
        </>
      }
    >
      ...
      <form className="ck-login-form" onSubmit={submit}>
        <label className="ck-field ck-field--icon">
          ...
          <input required placeholder="Your name" value={form.name} onChange={update("name")} autoComplete="name" />
        </label>
        <label className="ck-field ck-field--icon">
          <span>Email</span>
        ...
        <label className="ck-field ck-field--icon">
          ...
          <input required placeholder="Street address" value={form.address} onChange={update("address")} autoComplete="street-address" />
        </label>
        <LocationMapPicker
          label="Location (optional) — pick on map"
          value={
        ...
        />
        <label className="ck-field ck-field--icon">
          <span>Profile photo (optional, max 5 MB)</span>
          <i className="fa-solid fa-image" aria-hidden="true" />
          <input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={handlePhoto} />
        </label>
        {photoPreview && (
          <img src={photoPreview} alt="Profile preview" style={{ maxWidth: 96, borderRadius: 8, marginBottom: "0.5rem" }} />
        )}
        <label className="ck-field ck-field--icon">
          <span>Password</span>
